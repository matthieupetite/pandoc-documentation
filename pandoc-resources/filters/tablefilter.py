...
def tbl_headers(s):
    ...
    for i in range(1, len(s)):
        result.append(inlatex(r'} & \textbf{\color{tableheadcolor}'))
    ...
    result.insert(0, inlatex(r'\textbf{\color{tableheadcolor}'))
    # Preprend the command to set the row color in front of everything
    result.insert(0, inlatex(r'\rowcolor{tableheadbackcolor} '))
...
def do_filter(k, v, f, m):
    if k == "Table":
        ...
        return [latex(r'\begin{table}[h]'),
                latex(r'\taburulecolor{tablegridcolor}'),
                latex(r'\captionsetup{justification=centering}'),
                latex(r'\centering'),
